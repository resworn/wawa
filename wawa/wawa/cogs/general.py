...
class General(commands.Cog):
    ...
    async def on_ready(self):
        ...
        # Upload your icons to wawa/icons (file name will be the name of the command)

        guild = self.bot.guilds[0]
        guild_emojis = [emoji.name for emoji in guild.emojis]

        icons = glob.glob('wawa/assets/icons/*.png')
        ...
        roles_channel = self.bot.get_channel(wawa.config['ROLES_CHANNEL'])
        roles = wawa.config['roles']
    ...
    async def on_member_join(self, member):
        channel = self.bot.get_channel(wawa.config['WELCOME_CHANNEL'])
    ...
    async def on_member_remove(self, member):
        channel = self.bot.get_channel(wawa.config['GOODBYE_CHANNEL'])
    ...
    async def help(self, ctx):
        prefix = wawa.config["COMMAND_PREFIX"]
