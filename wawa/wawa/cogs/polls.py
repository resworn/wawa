...
class MySource(menus.ListPageSource):
    async def format_page(self, menu, entries):
        ...
        embed.set_footer(text=f"Requested by {menu.interaction.user}")
        return embed
...
class MyMenuPages(ui.View, menus.MenuPages):
    ...
    async def start(self, ctx, *, channel=None, wait=False):
        ...
        self.ctx = ctx
        self.message = await self.send_initial_message(ctx, interaction.channel)
...
class Polls(commands.Cog):
    ...
    async def poll(self, ctx, question: str):
        ...
        cursor.execute("INSERT INTO polls (id, user_id, question, channel, message) VALUES (%s, %s, %s, %s, %s)", (poll_id, interaction.user.id, question, interaction.channel.id, sent_message.id))
        db.commit()
